...
class printParser(HTMLParser):
    def __init__(self):
        ...
        self.sig = {
            ...
            "container":   ['html', 'body', 'form', 'div', 'div', 'div', 'p',
                            'small'],
            "difficulty" : ['html', 'body', 'form', 'div', 'div', 'div', 'p',
                            'img'],
            "diff_marker": ['html', 'body', 'form', 'div', 'div', 'div', 'p',
                            'strong'],
            "content":     ['html', 'body', 'form', 'div', 'div', 'div', 'div',
                            'div']
        }
    ...
    def handle_starttag(self, name, attrs):
        ...
        elif(self.stack == self.sig["difficulty"] and
             self.result["difficulty"] == "" and
             self.atDif):
            self.result["difficulty"] = [a[1].split()[0]
                                         for a in attrs if a[0] == 'alt'][0]
            self.atDif = False
        elif(self.stack == self.sig["difficulty"] and
             self.result["terrain"] == "" and
             self.atTer):
            self.result["terrain"] = [a[1].split()[0]
                                      for a in attrs if a[0] == 'alt'][0]
            self.atTer = False
        elif(self.stack == self.sig["content"] and
             (self.insDes or self.inlDes or self.inHint)):
            #print "Start recording" + " at: " + str(self.stack)
            self.recording = True
            self.backupStack = str(self.stack)
    
    def handle_endtag(self, name):
        ''' Handle EOF, recording of verbatim html and stack.'''
        self.entity = None
        if name == 'html':
            self.finish()
    ...
    def handle_data(self, data):
        ...
        elif(self.stack == self.sig["container"] and
